...
def mark_sha(db, sha):
    db.execute('insert or ignore into alive values (?)', (buffer(sha),))


def sha_marked(db, sha):
    result = db.execute('select 1 from alive where sha = ?', (buffer(sha),))
    return result.fetchone()

def find_live_objects(existing_count, cat_pipe, opt):
    pack_dir = git.repo('objects/pack')
    ffd, db_filename = tempfile.mkstemp('.sqlite', 'tmp-gc-', pack_dir)
    os.close(ffd)
    live_objs = sqlite3.connect(db_filename)
    c = live_objs.cursor()
    c.execute("""create table alive (sha blob primary key) without rowid""")
    def already_seen(x):
        return sha_marked(c, x.decode('hex'))
    for ref_name, ref_id in git.list_refs():
        for id, type in walk_object(cat_pipe, ref_id.encode('hex'), opt.verbose,
                                    ...
                                    stop_at=already_seen,
                                    include_data=None):
            mark_sha(c, id.decode('hex'))
    live_objs.commit()
    return (live_objs, db_filename)


def sweep(live_objects, existing_count, cat_pipe, opt):
    # Traverse all the packs, saving the (probably) live data.

    live_objects = live_objects.cursor()
    ns = Nonlocal()
    ns.stale_files = []
    ...
    for idx_name in glob.glob(os.path.join(git.repo('objects/pack'), '*.idx')):
        ...
        for i in xrange(0, len(idx)):
            ...
            if sha_marked(live_objects, sha):
                idx_live_count += 1
...
if not existing_count:
    ...
    live_objects, live_file = find_live_objects(existing_count, cat_pipe, opt)
    try:
